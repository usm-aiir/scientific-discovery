...
Pipeline:
 1. Load the three TSVs (captions, metadata, references) and join them.
 2. Resolve each figure's image path on disk (figures/{yy}/{mm}/{paper_id}/{fig}{sub}.png)
    and drop rows whose image file is missing.
 3. Build a diverse candidate pool:
      - at most ONE figure per paper
      - roughly balanced across arXiv categories
 ...
 4. For each candidate figure, run a two-agent loop with a local Gemma model:
      Agent 1 ("Author")   -> drafts a scientist's information need (a query)
                               answerable by looking at the figure.
      Agent 2 ("Reviewer") -> checks the query; either ACCEPTs it or REJECTs
                               it with concrete feedback for Agent 1 to fix.
    Up to 3 rounds. If still rejected after 3 rounds, the figure is skipped
    and we move on to the next candidate.
 5. Stop once we have 200 accepted queries (or the candidate pool runs out).
 6. Write paper_id, figure_id, sub_id, query to a TSV, incrementally (so the
    script is safely resumable if interrupted).
...
def load_data(captions_tsv, metadata_tsv, ref_tsv):
   ...
   # Collapse multiple reference_text rows per (paper_id, figure_id) into one string.
   df_ref_grouped = (
   ...
   # Drop rows with no caption at all -- nothing useful to build a query from.
...
def resolve_image_path(figures_dir, paper_id, figure_id, sub_id):
   """
   paper_id looks like '2410.24226' -> subfolders figures/24/10/2410.24226/...
   image filename is figure_id (+ sub_id if present) + '.png', e.g. '10a.png'
   """
...
def build_diverse_candidate_order(df, pool_multiplier=4, seed=13):
   """
   Returns a list of row-indices (into df) in the order they should be
   attempted, satisfying:
     - at most one figure per paper
     - round-robin across categories (keeps things balanced)
     - within a category, alternate across caption-length buckets


   The returned list is intentionally larger than the target (pool_multiplier x)
   so that we have enough backup candidates to absorb skips from the
   verification loop.
   """
   ...
   # Step 1: one figure per paper (randomly chosen among that paper's figures).
   one_per_paper = []
   ...
   # Step 2: bucket by category, and within category interleave length buckets.
   ...
   for cat, idxs in by_category.items():
       # group by length bucket, shuffle each, then interleave short/medium/long
   ...
   # Step 3: round-robin across categories so no single category dominates
   ...
   log.info(
       "Candidate pool built: %d papers across %d categories.",
       len(order), len(cat_names),
   )
   return order
...
# --------------------------------------------------------------------------
# 4. Model wrapper (Gemma) + the two agents
# --------------------------------------------------------------------------


class GemmaChat:
   """Thin wrapper around a local HF causal LM used as a chat model."""
...
# ---------- UPDATED PROMPTS (figure‑agnostic, abstract‑aware) ----------


AGENT1_SYSTEM = """You are simulating a working scientist who has just come across a figure in a paper (via its caption, sub‑caption, abstract, and how other papers cite it). Your job is to produce ONE realistic information need this scientist would have – a natural‑language query that can be answered by LOOKING AT THE FIGURE ITSELF (not by reading the rest of the paper).


Important constraints:
- Do NOT refer to the figure by its number or say "in the figure" or "Figure X". The query must be phrased as a standalone question that a researcher might ask, e.g. "How does the predicted interest rate change with increasing term length for each mechanism?"
- The query must be answerable using only the figure's visual content, given the context provided.
- It must be specific to this figure's content, not generic.
- It should be a natural, one to two sentence question, as if asked in a conversation or search.
...
AGENT2_SYSTEM = """You are a careful peer reviewer checking whether a proposed query is a good fit for a figure‑grounded question‑answering benchmark. ACCEPT the query only if ALL of the following hold:
 1. It can plausibly be answered by looking at the figure alone (given the caption/context you were shown), without needing the rest of the paper.
 2. It is specific to this particular figure (not a generic template question).
 3. It reads like something a real scientist would ask, and is grammatical, one to two sentences.
 4. It does not already state the answer/finding within the query itself.
 5. It is not simply restating the caption verbatim.
 6. It does NOT mention the figure number or say "in the figure" or "Figure X" – the query must be a standalone, figure‑agnostic question.


If any criterion fails, REJECT and give concise, actionable feedback describing exactly what to change (e.g. "too generic, ask about the specific trend for method X" or "the query gives away the answer, rephrase as a question instead of a statement" or "remove reference to Figure 3").


Respond with ONLY a JSON object:
{"verdict": "ACCEPT" or "REJECT", "feedback": "<empty string if ACCEPT, else the fix needed>"}"""




def figure_context_block(row):
   # Include abstract for better grounding
   abstract = row.get('abstract', '')
   if len(abstract) > 1500:  # truncate to avoid token blow
   ...
   return (
       ...
       + (f" (sub-figure {row['sub_id']})" if row["sub_id"] else "")
       + "\n"
       f"Figure caption: {row['caption']}\n"
       f"Sub-caption: {row['sub_caption']}\n"
       f"How the figure is referenced elsewhere in the text: {row['reference_text'] or '(none found)'}"
   )
...
def run_agent1(chat, row, prior_query=None, feedback=None):
   ...
   # Extra safety: remove any accidental "Figure X" references
...
def generate_query_for_figure(chat, row, max_rounds=3):
   ...
   for attempt in range(1, max_rounds + 1):
       ...
       if query is None:
           ...
           feedback = "Your previous response was not valid JSON with a 'query' field. Return only the JSON object."
           continue
...
def main():
   ...
   ap.add_argument("--pool_multiplier", type=int, default=4,
                    help="How many extra candidates (relative to target) to keep as backups for skips.")
   ...
   if already_done_papers:
       log.info("Resuming: %d papers already have accepted queries in %s", len(already_done_papers), args.output_tsv)
   ...
   for idx in order:
       ...
       n_attempted += 1
       log.info(
           "[%d accepted / %d target | %d attempted | %d skipped] paper=%s figure=%s%s",
       ...
       try:
           ...
           log.exception("Error while processing paper=%s figure=%s: %s", row["paper_id"], row["figure_id"], e)
           query = None
       ...
       # Safety: strip tabs/newlines so the TSV stays well-formed.
