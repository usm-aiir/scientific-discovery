...
Input files (auto-discovered under OUTPUT_DIR):
  ...
  metadata_<year>_<month>.tsv   — one row per paper  (arxiv_scraper.py)
...
Usage
-----
  # Use default OUTPUT_DIR (edit the constant below):
  python build_sample.py
 
  # Override the scraper output directory:
  python build_sample.py /path/to/arxiv_data
 
  # Also override the output file:
  python build_sample.py /path/to/arxiv_data /path/to/sampled_tables.tsv
"""
...
# Root directory that the scrapers wrote into (contains tables/ and metadata_*.tsv)
...
# (Each table record's "references" field is already a list; we join here.)
REFERENCE_SEPARATOR = " | "
...
def load_tables(output_dir: Path) -> pd.DataFrame:
    ...
    for path in jsonl_files:
        ...
        # read_json with lines=True handles JSONL (one JSON object per line).
    ...
    # e.g. "2410.00004_T1"  →  "2410.00004"
    #      "2410.00004_T14a" →  "2410.00004"
    tables_df["paper_id"] = tables_df["table_id"].str.extract(r"^(.+?)_T\d+", expand=False)
 
    # ---- Flatten the references list into a single string ----
    # Each record's "references" field is a Python list of paragraph strings.
...
def load_metadata(output_dir: Path) -> pd.DataFrame:
    """
    Load every ``metadata_<year>_<month>.tsv`` file under *output_dir* and
    return a single concatenated DataFrame.
 
    Columns written by arxiv_scraper.py:
        url, paper_id, title, abstract, categories
 
    The ``categories`` field uses ``"; "`` (semicolon) as the separator
    between multiple category labels (e.g. ``"cs.LG; cs.CL"``).
    """
    tsv_files = sorted(output_dir.glob("metadata_*.tsv"))
    if not tsv_files:
        raise FileNotFoundError(
            f"No metadata TSV files found under {output_dir}.\n"
            "Check that arxiv_scraper.py has finished writing output there."
        )
...
def merge_dataframes(
    ...
    (e.g. scraped before metadata was written) are still kept in the pool
    with blank title / abstract / category fields.
    """
    merged = tables_df.merge(metadata_df, on="paper_id", how="left")
 
    # Fill any remaining NaN in downstream columns.
...
def build_category_index(merged_df: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    arxiv_scraper.py stores multiple categories as a semicolon-separated
    string (e.g. ``"cs.LG; cs.CL"``).  Each table is placed into every
    category its paper belongs to, so one ``table_id`` can appear under
    multiple keys.
 
    Tables with no category go into a ``_uncategorised`` bucket so they
    remain eligible for sampling.
 
    Within each bucket, ``table_id`` values are deduplicated while
    ...
    for _, row in merged_df.iterrows():
        ...
        # arxiv_scraper.py joins categories with "; " (semicolon + space).
...
def _allocate_slots(
    ...
    """
    Proportionally divide *total* sampling slots across categories,
    guaranteeing at least 1 slot per category.
 
    Uses the Hamilton (largest-remainder) method so allocations sum to
    exactly *total* without systematic bias toward large or small categories.
 
    If there are more categories than slots, each eligible category gets at
    most 1 slot and the largest buckets take priority (a warning is logged).
    """
    ...
    # Raw proportional quotas (may be fractional).
    raw = [total * s / grand_total for s in sizes]
 
    # Floor each quota, guarantee minimum of 1.
...
def stratified_sample(
    ...
    Phase 1 — Proportional draw:
        Allocate slots across categories proportionally using Hamilton's
        method, then shuffle each bucket with a seeded RNG and take the
        first n_slots entries.
 
    Phase 2 — Gap fill:
        Because a single table can belong to multiple categories, Phase 1
        may produce duplicates that reduce the unique count below
        *sample_size*.  Any shortfall is filled by sampling uniformly from
        the remaining (unselected) pool.
 
    Both phases share the same seeded RNG instance, so the full procedure
    is reproducible given the same seed and input data.
 
    Returns a list of exactly *sample_size* unique table IDs, or fewer if
    the total pool is smaller than *sample_size*.
    """
    rng = random.Random(seed)
 
    # Full unique pool across all categories (used for gap-fill).
    ...
    for cat, n_slots in allocation.items():
        if n_slots == 0:
            continue
        bucket = list(category_index[cat])   # local copy; shuffle doesn't mutate the index
        rng.shuffle(bucket)
        selected.update(bucket[:n_slots])
 
    n_phase1 = len(selected)
    n_dupes   = sample_size - n_phase1
    log.info(
        "Phase 1: drew from %d categories → %d unique tables "
        "(target %d; %d cross-category duplicates removed).",
        len(allocation), n_phase1, sample_size, max(0, n_dupes),
    )
...
def build_output(
    ...
    Any OUTPUT_COLUMNS absent from the merged frame are added as empty
    strings to guarantee a consistent output schema.
    """
    # Build table_id → sorted, comma-joined category string.
    ...
    # Guard against duplicate rows introduced by the merge.
    before = len(subset)
    ...
    output = subset[OUTPUT_COLUMNS].reset_index(drop=True)
    return output
...
def write_output(output_df: pd.DataFrame, path: Path) -> None:
    ...
    log.info("Output written: %s (%d rows × %d columns).", path, *output_df.shape)
...
def main(
    ...
    """
    log.info(
        "=== build_sample.py  output_dir=%s  seed=%d  target=%d tables ===",
...
if __name__ == "__main__":
    # Positional arguments (all optional — falls back to the constants above):
    #   python build_sample.py [output_dir] [output.tsv]
    _args = sys.argv[1:]
    _kwargs: dict = {}
    if len(_args) >= 1: _kwargs["output_dir"]   = Path(_args[0])
    if len(_args) >= 2: _kwargs["output_path"]  = Path(_args[1])
    main(**_kwargs)